...
work with custom extensions. This section is an overview of the applications of custom extensions,
...
\subsection{Hardware Acceleration for Edge Machine Learning}

The deployment of deep learning models on resource-constrained microcontrollers,
often referred to as Edge AI or TinyML, heavily relies on the optimization of underlying arithmetic operations.
Neural network inference is dominated by matrix multiplication,
which at its core consists of repeated Multiply-Accumulate (MAC) operations.

Frameworks like TensorFlow Lite for Microcontrollers (TFLite Micro) \cite{David2021TFLiteMicro}
provide reference software implementations for these operations. However, purely software-based execution
on embedded processors often fails to meet strict latency and power constraints.
To address this, various works have proposed ISA extensions and co-processors.
For example, the CMSIS-NN library \cite{Lai2018CMSISNN} leverages ARM DSP extensions (like SIMD MAC instructions)
to accelerate neural network primitives, achieving up to a 4.6x performance improvement over baseline implementations.

Similarly, the PULP (Parallel Ultra-Low-Power) platform \cite{PULP2015} incorporates custom RISC-V DSP extensions, including hardware loops and dot-product instructions, to accelerate convolutional and fully connected layers. These approaches demonstrate that introducing specialized MAC instructions directly into the CPU pipeline can significantly reduce the cycle count for machine learning workloads without the overhead of offloading data to a heavy external accelerator.
