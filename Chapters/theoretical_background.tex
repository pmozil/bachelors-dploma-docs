...
A linker script specifies instruction memory region, data memory region and stack and heap placement.
...
Buildroot automates cross-toolchain generation, linux kernel config,
rootfs and package selection.
...
\begin{figure}[htb]
    ...
    \end{enumerate}
    \caption{VexiiRiscv CXU operation diagram.}
\end{figure}
...
an extension may be created to execute the arithmetic operation in a finite field in a single CPU cycle.
...
TensorFlow Lite for Microcontrollers (TFLite Micro) is a framework designed
to run machine learning models on microcontrollers and other devices with only
kilobytes of memory. To optimize memory footprint and execution speed,
models deployed via TFLite Micro are typically quantized,
converting 32-bit floating-point weights and activations into 8-bit integers (INT8).

\subsection{Multiply-Accumulate Operations}

In the models, the most computationally expensive layers,
such as Convolutional (Conv2D) and Fully Connected (Dense) layers,
are executed as a series of dot products. The fundamental mathematical
operation in a dot product is the Multiply-Accumulate (MAC) operation, defined as:

$$
acc = acc + (weight \times activation)
$$

In a standard RISC-V RV32IM implementation, performing a single MAC operation
with 32-bit accumulation requires fetching the operands,
performing a multiplication (e.g., \textbf{mul}), adding the result
to an accumulator register (e.g., \textbf{add}), and handling loop
overhead and memory loads. By introducing a custom hardware instruction
that combines multiplication and accumulation into a single computational step,
the instruction footprint and pipeline hazards within the inner
loop of the neural network kernel can be drastically minimized.
