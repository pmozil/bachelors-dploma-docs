...
As such, there is also no unified sandbox environment that allows developers
...
an RV32IMAFC configured CPU implemented in the SpinalHDL hardware description language.
...
\begin{itemize}
    ...
    \item \textbf{System-Level Benchmarking:} After being synthesized on the FPGA,
...
The data collection process for this work consists of two phases:
the implementation of the CXU interface itself,
...
the VexiiRiscv pipeline. This required implementing a
...
The first extension is an AES extension that offloads finite
...
runtime library. Both variants were compiled for the \textbf{RV32IMAFC}
...
overhead in the Linux environment.
